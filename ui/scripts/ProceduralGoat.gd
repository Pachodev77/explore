...
func _generate_structure():
	...
	neck_base.translation = chest_p + Vector3(0, hu*0.15, -hu*0.5) 
	body_root.add_child(neck_base); parts["neck_base"] = neck_base
	
	var n1_v = Vector3(0, hu*0.7, -hu*0.45) 
	...
	for s in [-1, 1]:
		...
		var h2_v = Vector3(0, hu*0.2, hu*0.35) 
	...
	for i in range(4):
		...
		if not is_f: u_v = Vector3(0, -hu*0.85, hu*0.15) 
		...
		if not is_f: lo_v = Vector3(0, -hu*0.8, -hu*0.2) 
